...
class ForecastExplainer:
    def __init__(
        self,
        model: Any,
        training_data: Union[np.ndarray],
        training_outputs: Union[np.ndarray] = None,  # Made optional with default None
    ...
    def predict(self, input_data: Union[np.ndarray]) -> np.ndarray:
        """
    ...
    def explain_prediction(
        ...
        exp = explainer.explain_instance(
            ...
            num_samples=1000,
            model_regressor=Ridge(fit_intercept=True, 
                                  random_state=123)
        )
        ...
        lemon_exp = lemon_explainer.explain_instance(input_data_flat, predict_fn, surrogate=Ridge(fit_intercept=True, random_state=123))[0]


        #explanation = exp.as_list()
        explanation = [(label, value) for label, value in zip(input_labels[:num_features], lemon_exp.feature_contribution)]

        return explanation, exp.score, lemon_exp.local_faithfulness, input_data_flat.shape
    ...
    def predict_and_explain(
        ...
        for i in range(n_predictions):
            ...
            xplain = self.explain_prediction(current_input, current_labels, num_features=num_features, kernel_width=kernel_width)
            explanation = xplain[0]
            lime_scores.append(xplain[1])
            lemon_scores.append(xplain[2])
